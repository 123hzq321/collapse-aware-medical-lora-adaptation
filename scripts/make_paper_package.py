...
在 0.5B 模型上，full45k self-update 在三个 seed 上稳定改善三项医学任务。PubMedQA 从 frozen baseline 的 {q05_pub['Frozen Acc.']} 提升到 {q05_pub['Full45k Acc.']}，并将 collapse 从 {q05_pub['Frozen Collapse']} 降到 {q05_pub['Full45k Collapse']}。MedMCQA 和 MedQA 也分别获得稳定提升，说明 full45k 不只是改变 dominant label，而是改善了多类覆盖。
...
For the 0.5B model, full45k self-update consistently improves all three medical QA tasks across three seeds. On PubMedQA, accuracy improves from {q05_pub['Frozen Acc.']} to {q05_pub['Full45k Acc.']}, while collapse is reduced from {q05_pub['Frozen Collapse']} to {q05_pub['Full45k Collapse']}. MedMCQA and MedQA also improve, indicating that the update improves multi-class coverage rather than merely shifting the dominant label.

Larger models are not automatically immune to collapse. In particular, the 3B frozen baseline still collapses on PubMedQA with a maybe-dominant prediction pattern. After full45k self-update, the 3B model reaches {q3_medmcqa['Full45k Acc.']} accuracy / {q3_medmcqa['Full45k Macro-F1']} macro-F1 on MedMCQA and {q3_medqa['Full45k Acc.']} accuracy / {q3_medqa['Full45k Macro-F1']} macro-F1 on MedQA. The best PubMedQA score is obtained by the 1.5B full45k model, with {q15_pub['Full45k Acc.']} accuracy / {q15_pub['Full45k Macro-F1']} macro-F1.
